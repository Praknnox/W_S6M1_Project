...
export default function Programmers() {
  ...
  const [grammers]=useState(listOfAwesome)
  const [selectYour,setGrammer]=useState(null)
  ...
  const getNameOfFeatured = () => {
    ...
    return grammers.find(prgr=>prgr.id===selectYour).name
  };

  const style = {
    fontSize: '1.5em',
    marginTop: '0.5em',
    color: selectYour?'gold':'royalblue', // 🤔 color turns to gold, when celebrating
  };
